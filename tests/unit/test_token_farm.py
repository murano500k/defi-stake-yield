...
def test_get_token_value(amount_staked):
    ...
    token_farm, dapp_token = deploy_token_farm_and_dapp_token()
    # Act
    (token_value, decimals) = token_farm.getTokenValue(dapp_token)
    # Arrange
...
def test_unstake_tokens(amount_staked):
    ...
    token_farm, dapp_token = test_stake_tokens(amount_staked)
    starting_balance = dapp_token.balanceOf(account.address)
    starting_unique_tokens = token_farm.uniqueTokensStaked(account)
    # Act
    ...
    assert token_farm.stakingBalance(dapp_token, account) == 0
    assert token_farm.uniqueTokensStaked(account) == (starting_unique_tokens - 1)
...
def test_add_allowed_tokens(amount_staked, random_erc20):
    ...
    token_farm, dapp_token = test_stake_tokens(amount_staked)
    # Act
    initial_length = token_farm.allowedTokens.length
    ...
    # Assert
    assert token_farm.allowedTokens.length == (initial_length + 1)
